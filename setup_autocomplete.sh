#!/bin/bash
# Скрипт для настройки автокомплита YCM (Объединенная база Linux & MinGW)
...
if [ -f "$BUILD_LINUX/compile_commands.json" ] && [ -f "$BUILD_WIN/compile_commands.json" ]; then
    ...
    echo "YCM will now use Linux flags for linux files and MinGW flags for windows files."
